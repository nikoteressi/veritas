...
class EventEmissionService:
    ...
    async def emit_reputation_retrieval_completed(self, username: str):
        ...
        await self.emit_event("REPUTATION_RETRIEVAL_COMPLETED", {
            "username": username
        })
    ...
    async def emit_fact_checking_started(self, total_claims: int):
        ...
        await self.emit_event("FACT_CHECKING_STARTED", {
    ...
    async def emit_fact_checking_item_completed(self, checked: int, total: int, claim_text: str):
        ...
        await self.emit_event("FACT_CHECKING_ITEM_COMPLETED", {
            "checked": checked,
            "total": total,
    ...
    async def emit_verdict_generation_completed(self, verdict: str):
        ...
        await self.emit_event("VERDICT_GENERATION_COMPLETED", {
            "verdict": verdict
        })
