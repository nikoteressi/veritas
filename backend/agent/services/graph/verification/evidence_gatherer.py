...
class EnhancedEvidenceGatherer:
    ...
    async def create_cluster_search_queries(self, cluster: FactCluster) -> list[str]:
        ...
        logger.info("Generated %d LLM queries for cluster %s: %s", len(queries), cluster.id, queries)
        return queries
    ...
    async def execute_searches_batch(
        ...
        logger.info("Executing %d search queries with context: %s", len(queries), query_context)
    ...
    async def gather_cluster_evidence(
        ...
        try:
            ...
            logger.info(
                f"Gathered evidence for cluster {cluster.id}: {len(search_queries)} queries, "
                f"{len(credible_sources)} credible sources, avg_query_relevance: {avg_query_relevance:.3f}"
            )
