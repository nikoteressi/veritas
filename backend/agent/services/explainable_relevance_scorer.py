...
class ExplainableRelevanceScorer:
    ...
    def _generate_recommendations(
        ...
        if len(text.split()) < 50:
            recommendations.append("Text might be too short for comprehensive analysis")
    ...
    def _calculate_confidence_metrics(
        ...
        if "bm25_score" in components and "semantic_score" in components:
            score_diff = abs(components["bm25_score"] - components["semantic_score"])
    ...
    async def explain_relevance(
        ...
        # Cache explanation
        if use_cache:
            dependencies = [f"query:{hashlib.md5(query.encode()).hexdigest()[:8]}"]
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        avg_processing_time = self.metrics["processing_time"] / total_explanations
