...
from agent.services.advanced_clustering import (
    AdvancedClusteringSystem,
    ClusteringConfig,
)
from agent.services.bayesian_uncertainty import (
    BayesianUncertaintyHandler,
    UncertaintyConfig,
)
from agent.services.graph_builder import GraphBuilder
from agent.services.graph_config import ClusteringConfig, VerificationConfig
from agent.services.graph_storage import Neo4jGraphStorage
from agent.services.graph_verification.engine import GraphVerificationEngine
from agent.services.intelligent_cache import get_general_cache, get_verification_cache
from agent.services.relationship_analysis import (
    RelationshipAnalysisEngine,
    RelationshipConfig,
)
from agent.services.source_reputation import SourceReputationSystem
...
class GraphFactCheckingService:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(f"Neo4j storage unavailable: {e}")
            self.graph_storage = None
        ...
        try:
            ...
            logger.warning(f"Source reputation unavailable: {e}")
            self.source_reputation = None
        ...
        try:
            ...
            if self.graph_storage:
                ...
                try:
                    ...
                    raise RuntimeError(f"Graph storage is unavailable: {e}") from e
            ...
            if not graph:
                ...
                if self.graph_storage and graph_id:
                    try:
                        ...
                        raise RuntimeError(f"Graph storage save failed: {e}") from e
            ...
            if self.graph_storage and graph_id:
                try:
                    ...
                    raise RuntimeError(f"Failed to update graph storage: {e}") from e
    ...
    def _convert_fact_result_to_claim_result(
        ...
        if cluster_result:
            cluster_type = cluster_result.metadata.get("cluster_type", "unknown")
    ...
    async def get_source_reputation_score(self, source_url: str) -> Optional[float]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to get source reputation: {e}")
            return None
    ...
    async def _apply_enhanced_clustering(self, graph, enhanced_clusters):
        ...
        try:
            ...
            self.logger.error(f"Failed to apply enhanced clustering: {e}")
            return graph
    ...
    async def _apply_relationship_analysis(self, graph, relationships):
        ...
        try:
            ...
            self.logger.error(f"Failed to apply relationship analysis: {e}")
            return graph
