...
class VerificationResponse(BaseModel):
    ...
    verdict: str = Field(description="Verification verdict: TRUE, FALSE, PARTIALLY_TRUE, or INSUFFICIENT_EVIDENCE")
    ...
    evidence_used: list[str | dict] = Field(
        default_factory=list,
        description="List of evidence pieces used in verification (can be strings or objects with source/reasoning)",
    )
    supporting_evidence: int = Field(default=0, description="Number of supporting evidence pieces")
    contradicting_evidence: int = Field(default=0, description="Number of contradicting evidence pieces")


class SourceSelectionResponse(BaseModel):
    """Response model for source selection from search results."""

    selected_sources: list[int] = Field(description="List of selected source indices (1-based)")
    reasoning: str = Field(description="Brief explanation of why these sources were selected")
...
class Contradiction(BaseModel):
    ...
    verdict: str = Field(description="Verdict for the claim: TRUE, FALSE, PARTIALLY_TRUE, etc.")
    reasoning: str = Field(description="Reasoning for the contradiction detection")
    type: str = Field(description="Type of contradiction: direct, indirect, temporal, etc.")
    confidence: float = Field(description="Confidence in contradiction detection", ge=0.0, le=1.0)
...
class ContradictionDetectionResponse(BaseModel):
    ...
    overall_consistency: str = Field(description="Overall consistency assessment: CONSISTENT, INCONSISTENT, or MIXED")
    confidence: float = Field(description="Confidence in contradiction detection", ge=0.0, le=1.0)
...
class CrossVerificationResponse(BaseModel):
    ...
    relationship: str = Field(description="Relationship type: SUPPORTING, CONTRADICTING, or INDEPENDENT")
    confidence: float = Field(description="Confidence in relationship assessment", ge=0.0, le=1.0)
    reasoning: str = Field(description="Explanation of the relationship")
    evidence_overlap: list[str] = Field(default_factory=list, description="Overlapping evidence between facts")


class ClusterVerificationResponse(BaseModel):
    """Response model for cluster-level verification."""

    overall_verdict: str = Field(description="Overall cluster verdict: TRUE, FALSE, MIXED, or INSUFFICIENT_EVIDENCE")
...
class EvidenceAnalysisResponse(BaseModel):
    ...
    relevance_score: float = Field(description="Relevance score of evidence to claim", ge=0.0, le=1.0)
    credibility_score: float = Field(description="Credibility score of evidence source", ge=0.0, le=1.0)
    key_points: list[str] = Field(default_factory=list, description="Key points extracted from evidence")
