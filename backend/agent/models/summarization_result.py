...
class SummarizationResult(BaseModel):
    ...
    sources_used: List[str] = Field(
        ...
        description="Sources used for creating summary"
    )
    key_points: List[str] = Field(
        default_factory=list,
        description="Key points highlighted in summary"
    )
    temporal_context_included: bool = Field(
        default=False,
        description="Whether temporal context is included in summary"
    )
    ...
    claims_analyzed: int = Field(
        ge=0,
        description="Number of analyzed claims"
    )
