...
class Settings(BaseSettings):
    ...
    delay_before_return_html: int = Field(
        default=5000, env="DELAY_BEFORE_RETURN_HTML")  # 5 seconds in ms
    ...
    max_file_size: int = Field(
        default=10 * 1024 * 1024, env="VERITAS_MAX_FILE_SIZE")  # 10MB
    ...
    request_timeout_seconds: int = Field(
        default=300, env="VERITAS_REQUEST_TIMEOUT")  # 5 minutes
    fact_checking_timeout_seconds: int = Field(
        default=120, env="VERITAS_FACT_CHECK_TIMEOUT")  # 2 minutes
    ...
    def get_step_display_name(self, step) -> str:
        """Get display name for a pipeline step."""
        # Handle both VerificationSteps enum and string inputs
        ...
        return display_names.get(step_value, step_value.replace("_", " ").title())

    def get_progress_message(self, step) -> str:
        ...
        return progress_messages.get(step_value, f"Processing {step_value}...")
